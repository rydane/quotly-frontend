...
async function sendCodeEmail(email, name, code, subject, msg) {
  ...
  await t.sendMail({
    from: process.env.EMAIL_FROM || `Defact <${process.env.GMAIL_USER}>`,
    to: email, subject,
    html: `<div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden">
      <div style="background:#0e0d0b;padding:24px 32px;text-align:center"><h1 style="color:#c9a84c;margin:0;letter-spacing:.15em">DEFACT</h1></div>
      ...
      <div style="background:#f9f9f9;padding:16px;text-align:center;border-top:1px solid #eee"><p style="color:#aaa;font-size:12px;margin:0">© 2025 DEFACT</p></div>
    </div>`,
  });
}
...
router.post('/login', async (req, res) => {
  try {
    ...
    try { await sendCodeEmail(user.email, user.name, otp, `🔐 Code de connexion DEFACT : ${otp}`, 'Voici votre code de connexion, valable <strong>10 minutes</strong>.'); }
    catch (e) { console.error('OTP email:', e.message); if (process.env.NODE_ENV !== 'production') console.log(`[DEV] OTP ${user.email}: ${otp}`); }
...
router.post('/forgot-password', async (req, res) => {
  try {
    ...
    try { await sendCodeEmail(email.toLowerCase(), user.name, code, `🔑 Code de réinitialisation DEFACT : ${code}`, 'Voici votre code de réinitialisation, valable <strong>15 minutes</strong>.'); }
    catch (e) { console.error('Reset email:', e.message); if (process.env.NODE_ENV !== 'production') console.log(`[DEV] Reset code ${email}: ${code}`); }
