...
async function initSchema() {
  ...
  // ─── Migration critique : supprime la FK users.team_id → teams(id) ──────────
  // Le code utilise l'id du propriétaire directement comme team_id dans users,
  // pas un ID de la table teams. La contrainte FK causait donc un code 23503
  // (violation de clé étrangère) à chaque création de membre → erreur 500.
  // On transforme team_id en champ TEXT libre (auto-référence vers users.id).
  try {
    await db.query('ALTER TABLE users DROP CONSTRAINT IF EXISTS users_team_id_fkey');
    console.log('[migration] ✓ Contrainte FK users.team_id supprimée (team_id est maintenant libre)');
  } catch (e) {
    if (!e.message.includes('does not exist')) {
      console.warn('[migration] users_team_id_fkey drop:', e.message);
    }
  }

  // ─── Migration : team_members → users ────────────────────────────────────────
  // Les anciens comptes créés via team-routes.js (table team_members, déconnectée
  // de l'auth) sont migrés vers la table users pour qu'ils soient authentifiables.
  try {
    ...
    `);
    if (orphans.length > 0) {
      console.log(`[migration] Migrating ${orphans.length} team_member(s) into users table…`);
      for (const tm of orphans) {
        // Vérifie que l'owner existe toujours
        ...
        console.log(`[migration] ✓ ${tm.email} migré vers users (team_id=${tm.owner_id})`);
      }
    }
  } catch (e) {
    // La table team_members peut ne pas exister sur les nouvelles installations
    if (!e.message.includes('does not exist')) {
