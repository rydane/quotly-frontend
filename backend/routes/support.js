...
const ADMIN_EMAILS = ['berkiadam92@gmail.com', 'rydane.j@gmail.com'];

function isAdmin(email) {
  return ADMIN_EMAILS.includes(email?.toLowerCase());
}

// POST /api/support/message — tout utilisateur authentifié peut envoyer un message
router.post('/message', requireAuth, async (req, res) => {
  try {
    ...
    res.status(201).json({ id, message: 'Message envoyé au support.' });
  } catch (err) {
...
// GET /api/support/my-messages — utilisateur : voir ses propres messages + réponses admin
...
router.get('/messages', requireAuth, async (req, res) => {
  try {
    if (!isAdmin(req.user.email)) return res.status(403).json({ error: 'Accès refusé.' });
...
// POST /api/support/messages/:id/reply — admin uniquement : répondre à un message
router.post('/messages/:id/reply', requireAuth, async (req, res) => {
  try {
    ...
    // Ajouter la réponse au tableau JSONB replies
    ...
    res.json({ message: 'Réponse envoyée.', replies });
  } catch (err) {
